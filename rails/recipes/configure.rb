...
node[:deploy].each do |application, deploy|
    ...
    template "#{deploy[:deploy_to]}/shared/config/analyst.yml" do
        source "analyst.yml.erb"
        cookbook 'rails'
        mode "0660"
        group deploy[:group]
        owner deploy[:user]
    ...
    template "#{deploy[:current_path]}/config/analyst.yml" do
        source "analyst.yml.erb"
        cookbook 'rails'
        mode "0660"
        group deploy[:group]
        ...
        only_if do
            File.exists?("#{deploy[:current_path]}") && File.exists?("#{deploy[:current_path]}/config/")
        end
    end
